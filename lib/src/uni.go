...
// the first unification algorithm being developed by Robinson. More efficient algorithms were developed later by Martelli and, Montanari.
...
func Unify(tokens []*Token, ix, iy int) map[string]string {
	//tokens := Tokenize(x, y)
	ut := New(tokens)
